...
if (@$taxList == 1) {
  my $taxObj = $taxList->[0];
  print redirect(-url => "taxon.cgi?level=$taxObj->{level}&taxon=$taxObj->{taxon}");
  exit(0);
}

start_page('title' => "Taxon search");
if (@$taxList == 0 && $query ne "") {
  print p("Sorry, no matching taxa were found for", '"' . encode_entities($query) . '"');
}
...
if (@$taxList > 0) {
  ...
  foreach my $taxObj (@$taxList) {
    ...
    print Tr({-bgcolor => $iRow % 2 == 0 ? "white" : "lightgrey" },
             td($taxObj->{level}),
             td(a({-href => "taxon.cgi?level=$taxObj->{level}&taxon=$taxObj->{taxon}" },
